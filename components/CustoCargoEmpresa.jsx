    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-slate-100 py-12 px-4">
      ...
      <div className="max-w-6xl mx-auto">
        <div className="bg-white rounded-lg shadow-md p-8 mb-8 animate-fade">
          ...
          <input type="number" value={salario} onChange={(e) => setSalario(parseFloat(e.target.value) || 0)} className="w-full px-4 py-3 border-2 border-slate-300 rounded-lg focus:outline-none focus:border-blue-500 text-lg font-semibold text-slate-800" aria-label="Digite o salario" />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          <div className="bg-white rounded-lg shadow-md p-8 animate-fade" style={{animationDelay: '0.2s'}}>
            ...
            {encargosData.length > 0 && (
              <ResponsiveContainer width="100%" height={300}>
                <PieChart>
                  <Pie data={encargosData} cx="50%" cy="50%" labelLine={false} label={({name, value}) => `${name}: ${((value / custoTotal) * 100).toFixed(1)}%`} outerRadius={100} fill="#8884d8" dataKey="value">
                    {encargosData.map((entry, idx) => (<Cell key={`cell-${idx}`} fill={entry.cor} />))}
                  </Pie>
                </PieChart>
              </ResponsiveContainer>
            )}
          ...
          <div className="bg-gradient-to-br from-blue-500 to-slate-700 rounded-lg shadow-lg p-8 text-white animate-fade" style={{animationDelay: '0.3s'}}>
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center border-b border-white border-opacity-20 pb-4"><span className="text-lg">Salario Base:</span><span className="text-2xl font-bold">R$ {salario.toLocaleString('pt-BR', {minimumFractionDigits: 2})}</span></div>
              <div className="flex justify-between items-center border-b border-white border-opacity-20 pb-4"><span className="text-lg">Encargos Totais:</span><span className="text-2xl font-bold">R$ {totalEncargos.toLocaleString('pt-BR', {minimumFractionDigits: 2})}</span></div>
              <div className="flex justify-between items-center pb-4"><span className="text-lg">% de Encargos:</span><span className="text-2xl font-bold">{percentualTotal}%</span></div>
        ...
        <div className="bg-white rounded-lg shadow-md p-8 mb-8 animate-fade" style={{animationDelay: '0.4s'}}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-8">
            {[{nome: 'Ana Luiza', cor: '#e0e7ff'}, {nome: 'Priscila', cor: '#dbeafe'}, {nome: 'David Bringel', cor: '#e0f2fe'}, {nome: 'Marvel', cor: '#f0f9ff'}, {nome: 'Kaue Santos', cor: '#dbeafe'}, {nome: 'Victor', cor: '#e0e7ff'}, {nome: 'Pedro Sales', cor: '#f3e8ff'}].map((membro, idx) => (
              <div key={idx} className="rounded-lg p-4 text-center font-semibold text-slate-700 shadow-sm border border-slate-200 hover:shadow-md transition-shadow" style={{backgroundColor: membro.cor}}>{membro.nome}</div>
            ))}
          </div>
          <div className="border-t-2 border-slate-200 pt-6 space-y-3">
            <div className="flex items-start"><span className="font-semibold text-slate-700 mr-3">Professor Orientador:</span><span className="text-slate-600">Rhubens Ewald Moura Ribeiro</span></div>
            <div className="flex items-start"><span className="font-semibold text-slate-700 mr-3">Grupo:</span><span className="text-slate-600">Ostentacao Prime</span></div>
            <div className="flex items-start"><span className="font-semibold text-slate-700 mr-3">Localizacao:</span><span className="text-slate-600">Teresina - PI</span></div>
          </div>
        </div>
      </div>
